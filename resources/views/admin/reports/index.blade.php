        ...
        <div class="sparkline8-list shadow-reset mg-tb-30">
            <div class="sparkline8-hd">
                <div class="main-sparkline8-hd" style="display: flex; justify-content: space-between;">
                    <h1>گزارش ها{{isset($kind) ? 'ی ' . $kind->name : ''}}</h1>
                    @if(isset($kind))
                        <button class="btn btn-success" data-toggle="modal" data-target="#newModal">گزارش جدید</button>

                        <button class="btn btn-warning" onclick="document.location.href = '{{url('reports')}}'">بازگشت</button>
                    @endif
                </div>
            </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($reports as $item)
                                <tr>
                                    ...
                                    <td class="center">
                                        <button class="btn btn-info width-auto center" onclick="editModal({{$item->id}})" title="ویرایش گزارش" >
                                            <span class="glyphicon glyphicon-edit mg-tp-30per"></span>
                                        </button>
                                        <button class="btn btn-danger width-auto center" onclick="deleteModal({{$item->id}})" title="حذف گزارش">
        ...
        <div class="modal fade" id="editModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('reports.doEdit')}}" method="post">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="row" style="padding: 20px">
                                <div class="form-group">
                                    <label for="editName">گزارش</label>
        ...
        function editModal(_id){
            ...
            for(i = 0; i < reports.length; i++){
                if(reports[i]['id'] == _id){
                    ...
                    document.getElementById('editName').value = reports[i]['description'];
                }
            }
        ...
        function deleteModal(_id){
            for(i = 0; i < reports.length; i++){
                if(reports[i]['id'] == _id){
                    ...
                    document.getElementById('deleteName').innerText = reports[i]['description'];
                }
            }
