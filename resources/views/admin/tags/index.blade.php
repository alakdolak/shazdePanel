        ...
        <div class="sparkline8-list shadow-reset mg-tb-30">
            <div class="sparkline8-hd">
                <div class="main-sparkline8-hd" style="display: flex; justify-content: space-between;">
                    <h1>تگ ها{{isset($kind) ? 'ی ' . $kind->name : ''}}</h1>
                    @if(isset($kind))
                        <button class="btn btn-success" data-toggle="modal" data-target="#newModal">تگ جدید</button>

                        <button class="btn btn-warning" onclick="document.location.href = '{{url('tags')}}'">بازگشت</button>
                    @endif
                </div>
            </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($tags as $item)
                                <tr>
                                    ...
                                    <td class="center">
                                        <button name="editDescription" class="btn btn-info width-auto center" onclick="editModal({{$item->id}})" title="ویرایش تگ" >
                                            <span class="glyphicon glyphicon-edit mg-tp-30per"></span>
                                        </button>
                                        <button name="deleteDescription" class="btn btn-danger width-auto center" onclick="deleteModal({{$item->id}})" title="حذف تگ">
        ...
        <div class="modal fade" id="editModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('tags.doEdit')}}" method="post">
                        @csrf
