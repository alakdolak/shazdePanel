...
@section('content')
        ...
        <div class="sparkline8-list shadow-reset mg-tb-30">
            <div class="sparkline8-hd">
                <div class="main-sparkline8-hd" style="display: flex; justify-content: space-between;">
                    <h1>آیتم ها{{isset($kind) ? 'ی ' . $kind->name : ''}}</h1>
                    @if(isset($kind))
                        <button class="btn btn-success" data-toggle="modal" data-target="#newModal">آیتم جدید</button>

                        <button class="btn btn-warning" onclick="document.location.href = '{{url('picItems')}}'">بازگشت</button>
                    @endif
                </div>
            </div>
                ...
                <center class="row" style="padding: 10px;">
                    @if(isset($kind))
                        <table class="table table-striped">
                            <tbody>
                            @foreach($picItems as $item)
                                <tr>
                                    <td style="text-align: center">
                                        {{$item->name}}
                                    </td>
                                    <td class="center">
                                        <button name="editDescription" class="btn btn-info width-auto center" onclick="editModal({{$item->id}})" title="ویرایش آیتم" >
                                            <span class="glyphicon glyphicon-edit mg-tp-30per"></span>
                                        </button>
                                        <button name="deleteDescription" class="btn btn-danger width-auto center" onclick="deleteModal({{$item->id}})" title="حذف آیتم">
        ...
        <div class="modal fade" id="newModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('picItems.store')}}" method="post">
        ...
        <div class="modal fade" id="editModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('picItems.doEdit')}}" method="post">
                        @csrf
        ...
        <div class="modal fade" id="deleteModal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form action="{{route('picItems.delete')}}" method="post">
                        @csrf
    ...
    <script>
        ...
        var picItems = {!! $picItems !!};

        function editModal(_id){

            for(i = 0; i < picItems.length; i++){
                if(picItems[i]['id'] == _id){
                    document.getElementById('editId').value = _id;
                    document.getElementById('editName').value = picItems[i]['name'];
                }
            }
        ...
        function deleteModal(_id){
            for(i = 0; i < picItems.length; i++){
                if(picItems[i]['id'] == _id){
                    ...
                    document.getElementById('deleteName').innerText = picItems[i]['name'];
                }
            }
