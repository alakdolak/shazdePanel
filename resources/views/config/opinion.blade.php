        ...
        <div class="sparkline8-list shadow-reset mg-tb-30">
            <div class="sparkline8-hd">
                <div class="main-sparkline8-hd" style="display: flex; justify-content: space-between;">
                    <h1>نظر ها{{isset($kind) ? 'ی ' . $kind->name : ''}}</h1>
                    @if(isset($kind))
                        <button class="btn btn-success" data-toggle="modal" data-target="#newModal">نظر جدید</button>

                        <button class="btn btn-warning" onclick="document.location.href = '{{url('opinions')}}'">بازگشت</button>
                    @endif
                </div>
            </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($opinions as $item)
                                <tr>
                                    ...
                                    <td class="center">
                                        <button name="editDescription" class="btn btn-info width-auto center" onclick="editModal({{$item->id}})" title="ویرایش نظر" >
                                            <span class="glyphicon glyphicon-edit mg-tp-30per"></span>
                                        </button>
                                        <button name="deleteDescription" class="btn btn-danger width-auto center" onclick="deleteModal({{$item->id}})" title="حذف نظر">
        ...
        <div class="modal fade" id="editModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('opinions.doEdit')}}" method="post">
                        @csrf
        ...
        <div class="modal fade" id="deleteModal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form action="{{route('opinions.delete')}}" method="post">
                        @csrf
    ...
    <script>
        ...
        function editModal(_id){
            ...
            for(i = 0; i < opinions.length; i++){
                if(opinions[i]['id'] == _id){
                    document.getElementById('editId').value = _id;
                    document.getElementById('editName').value = opinions[i]['name'];
                }
            }
        ...
        function deleteModal(_id){
            for(i = 0; i < opinions.length; i++){
                if(opinions[i]['id'] == _id){
                    ...
                    document.getElementById('deleteName').innerText = opinions[i]['name'];
                }
            }
