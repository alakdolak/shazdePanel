...
@section('content')
        ...
        <div class="sparkline8-list shadow-reset mg-tb-30">
            <div class="sparkline8-hd">
                <div class="main-sparkline8-hd" style="display: flex; justify-content: space-between;">
                    <h1>سبک مکان ها{{isset($kind) ? 'ی ' . $kind->name : ''}}</h1>
                    @if(isset($kind))
                        <button class="btn btn-success" data-toggle="modal" data-target="#newModal">سبک مکان  جدید</button>

                        <button class="btn btn-warning" onclick="document.location.href = '{{url('placeStyle')}}'">بازگشت</button>
                    @endif
                </div>
            </div>
                ...
                <center class="row" style="padding: 10px;">
                    @if(isset($kind))
                        <table class="table table-striped">
                            <tbody>
                            @foreach($placeStyle as $item)
                                <tr>
                                    <td style="text-align: center">
                                        {{$item->name}}
                                    </td>
                                    <td class="center">
                                        <button name="editDescription" class="btn btn-info width-auto center" onclick="editModal({{$item->id}})" title="ویرایش سبک مکان " >
                                            <span class="glyphicon glyphicon-edit mg-tp-30per"></span>
                                        </button>
                                        <button name="deleteDescription" class="btn btn-danger width-auto center" onclick="deleteModal({{$item->id}})" title="حذف سبک مکان ">
                    ...
                    @elseif(isset($places))
                        @foreach($places as $item)
                            <a href="{{URL('placeStyle/'.$item->name)}}">
        ...
        <div class="modal fade" id="newModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('placeStyle.store')}}" method="post">
        ...
        <div class="modal fade" id="editModal">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <form action="{{route('placeStyle.doEdit')}}" method="post">
                        @csrf
        ...
        <div class="modal fade" id="deleteModal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form action="{{route('placeStyle.delete')}}" method="post">
                        @csrf
    ...
    <script>
        ...
        function editModal(_id){
            ...
            for(i = 0; i < placeStyle.length; i++){
                if(placeStyle[i]['id'] == _id){
                    document.getElementById('editId').value = _id;
                    document.getElementById('editName').value = placeStyle[i]['name'];
                }
            }
        ...
        function deleteModal(_id){
            for(i = 0; i < placeStyle.length; i++){
                if(placeStyle[i]['id'] == _id){
                    ...
                    document.getElementById('deleteName').innerText = placeStyle[i]['name'];
                }
            }
